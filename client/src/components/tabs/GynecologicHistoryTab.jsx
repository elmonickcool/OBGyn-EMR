    ...
    <Grid item xs={12} sm={span}>
      <Box sx={{
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
          ...
          <Typography sx={{ fontSize: 10.5, fontWeight: 700, color: accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {label}
          </Typography>
        </Stack>
        {children}
  ...
  return (
    <Box
      ...
      sx={{
        ...
        px: 2, py: 1.2, borderRadius: 2.5, cursor: "pointer", userSelect: "none",
    ...
    >
      <Box sx={{
        ...
        {checked && <Typography sx={{ fontSize: 10, color: "#fff", fontWeight: 900, lineHeight: 1 }}>✓</Typography>}
      </Box>
      <Typography sx={{ fontSize: 13, fontWeight: checked ? 700 : 500, color: checked ? accent : t.ink }}>
        {label}
      </Typography>
    </Box>
  );
...
function GynecologicHistoryTab({ patient, form, setForm }) {
  ...
  const saveGynecologicHistory = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Failed to save gynecologic history");
    ...
    <Box>
      ...
      <Box sx={{
        ...
        <Stack direction="row" spacing={1.2} alignItems="center">
          ...
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: 14, color: t.fuchsiaDk }}>Gynecologic History</Typography>
        ...
        <Chip
          ...
          sx={{
            fontWeight: 800, fontSize: 13, height: 30, borderRadius: 2,
      ...
      {/* ── Menstrual History ───────────────────────────────────── */}
      <SubsectionHeader emoji="🩸" title="Menstrual History" accent={t.fuchsia} />
      <Grid container spacing={1.5}>
        <FieldCard emoji="📅" label="Last Menstrual Period" accent={t.fuchsia}>
          <TextField
            type="date" fullWidth variant="outlined"
        ...
        <FieldCard emoji="🌱" label="Age at Menarche" accent={t.violet}>
          <TextField
            type="number" fullWidth variant="outlined" placeholder="e.g. 12"
        ...
        <FieldCard emoji="⏱️" label="Cycle Duration (days)" accent={t.sky}>
          <TextField
            type="number" fullWidth variant="outlined" placeholder="e.g. 28"
      ...
      {/* ── Obstetric History ───────────────────────────────────── */}
      <SubsectionHeader emoji="🤰" title="Obstetric History (GPAL)" accent={t.violet} />
      <Grid container spacing={1.5}>
      ...
      <SubsectionHeader emoji="🍼" title="Delivery & Contraception" accent={t.teal} />
      <Grid container spacing={1.5}>
        ...
        <FieldCard emoji="🛡️" label="Contraception" accent={t.teal}>
          <TextField
            fullWidth variant="outlined" placeholder="e.g. None, OCP, IUD, Condom…"
