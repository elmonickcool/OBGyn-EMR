...
function avatarColor(id) {
  return avatarPool[Number(id) % avatarPool.length];
}
...
function StatPill({ label, value, bg, fg }) {
  return (
    ...
    >
      <Typography
        sx={{ fontSize: 22, fontWeight: 900, color: fg, lineHeight: 1 }}
      >
        {value}
      </Typography>
      <Typography
...
function Patients() {
  ...
  const patientsAddedThisMonth = patients.filter((patient) => {
    const createdDate = dayjs(patient.created_at);

    return (
      createdDate.month() === dayjs().month() &&
      createdDate.year() === dayjs().year()
    );
  }).length;
  ...
  useEffect(() => {
    const fetchFollowUps = async () => {
      try {
        const res = await fetch(`${API_URL}/consultation-followups`);
  ...
  const filtered = patients.filter((p) => {
    ...
    return (
      name.includes(query.toLowerCase()) || String(p.patient_id).includes(query)
    );
  });
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/patients/${deleteId}`, {
        method: "DELETE",
      });
    ...
    return (
      ...
      >
        <Box sx={{ textAlign: "center" }}>
          ...
          <Typography sx={{ color: t.midgray, fontWeight: 600 }}>
    ...
    >
      <Container maxWidth="lg" sx={{ pt: 3 }}>
        ...
        >
          ...
          <Box
            sx={{ px: { xs: 3, md: 5 }, pt: 4, pb: 3.5, position: "relative" }}
          >
            <Stack
            ...
            InputProps={{
              ...
              startAdornment: (
                <InputAdornment position="start">
                  <Typography sx={{ fontSize: 20, mr: 0.5, mt: 0.2 }}>
                    🔍
                  </Typography>
                </InputAdornment>
              ),
        ...
        >
          {filtered.length === 0 ? (
            <Box sx={{ py: 10, textAlign: "center" }}>
              ...
              <Typography sx={{ fontWeight: 700, fontSize: 16, color: t.ink }}>
                No patients found
              </Typography>
              <Typography sx={{ color: t.midgray, mt: 0.5 }}>
                ...
                <TableHead>
                  ...
                  >
                    {[
                      ...
                      { label: "Age", w: 80 },
                      { label: "Contact", w: 160 },
                      { label: "Last Visit", w: 120 },
                      { label: "Follow-up", w: 120 },
                      { label: "Actions", w: 220 },
                    ].map(({ label, w }) => (
                      <TableCell
                        key={label}
                ...
                <TableBody>
                  {filtered.map((patient, idx) => {
                    const nextFollowUp = followUps
                      .filter((fu) => fu.patient_id === patient.patient_id)
                      .sort(
                        (a, b) =>
                          new Date(a.follow_up_date) -
                          new Date(b.follow_up_date),
                      )[0];
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell sx={{ py: 1.8 }}>
                          {nextFollowUp ? (
                            <Chip
                              label={dayjs(nextFollowUp.follow_up_date).format(
                                "MMM D, YYYY",
                              )}
                              size="small"
                              sx={{
                                ...
                                backgroundColor: t.fuchsiaLt,
                                color: t.fuchsiaDk,
                                border: `1.5px solid ${t.fuchsia}33`,
                                borderRadius: 1.5,
                              }}
                            />
                          ) : (
        ...
        >
          ...
          <DialogContent>
            <DialogContentText sx={{ color: t.midgray, fontSize: 14 }}>
              This permanently removes the patient and all associated data. This
          ...
          <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
            <Button
              ...
              sx={{ borderRadius: 2.5, fontWeight: 700, color: t.midgray }}
            >
              Cancel
            </Button>
            <Button
